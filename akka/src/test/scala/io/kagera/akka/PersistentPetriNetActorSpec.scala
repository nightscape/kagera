...
object PersistentPetriNetActorSpec {
  ...
  def stateTransition[S, E](eventSourcing: S ⇒ E ⇒ S, fn: S ⇒ E, id: Long = Math.abs(Random.nextLong), label: String = "",
    isManaged: Boolean = false, exceptionStrategy: TransitionExceptionHandler = (e, n) ⇒ BlockSelf): Transition[Unit, E, S] =
    new AbstractTransition[Unit, E, S](id, label, isManaged, Duration.Undefined, exceptionStrategy) with UncoloredTransition[Unit, E, S] {
...
class PersistentPetriNetActorSpec extends TestKit(ActorSystem("test", PersistentPetriNetActorSpec.config))
  ...
  "A persistent petri net actor" should {
    ...
    "Retry to execute a transition with a delay when the exception strategy indicates so" in {
      ...
      val handler: TransitionExceptionHandler = (e, n) ⇒ (e, n) match {
        case (e, n) if n < 3 ⇒ RetryWithDelay((10 * Math.pow(2, n)).toLong)
        case _               ⇒ Fatal
      }

      val t1 = stateTransition[Set[Int], Event](eventSourcing,
        set ⇒ {
