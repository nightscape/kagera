...
class PetriNetInstanceSpec extends TestKit(ActorSystem("PetriNetInstanceSpec", PetriNetInstanceSpec.config))
  ...
  "A persistent petri net actor" should {
    ...
    "Respond with a TransitionNotEnabled message if a transition is not enabled because of not enough consumable tokens" in new StateTransitionNet[Set[Int], Event] {
    ...
    "fire automatic transitions in parallel when possible" in new StateTransitionNet[Unit, Unit] {
      ...
      failAfter(Span(1000, Milliseconds)) {
        ...
        expectMsgInAnyOrderPF(
          { case TransitionFired(t2.id, _, _, _) ⇒ },
          { case TransitionFired(t3.id, _, _, _) ⇒ }
        )
      }
    }
  }
