...
trait PetriNetQuery[S] {
  ...
  implicit class SourceAdditions[+Out, +Mat](source: Source[Out, Mat]) {
    def foldMap[S, E](zero: S)(fn: (S, Out) => (S, E)): Source[E, Mat] =
      source.scan[(S, E)]((zero, null.asInstanceOf[E])) {
        case ((s, prev), e) => fn(s, e)
      }.map(_._2)
  }
  ...
  def events(processId: String, topology: ExecutablePetriNet[S]): Source[Event, NotUsed] = {
    val src = readJournal.currentEventsByPersistenceId(PetriNetInstance.petriNetInstancePersistenceId(processId), 0, Long.MaxValue)

    src.foldMap(Instance.uninitialized[S](topology)) {
      (instance, e) =>
