...
class PetriNetActor(id: java.util.UUID, process: ColoredTokenGame, initialMarking: ColoredMarking) extends Actor with ActorLogging {
    ...
    case Step ⇒
      TokenGame.stepRandom[Place, Transition, ColoredMarking](process, marking) match {
        ...
        case Some((consume, t)) ⇒
          ...
          val produce =
            ColoredExecutor.executeTransition(process)(consume, t, None, id).map(
