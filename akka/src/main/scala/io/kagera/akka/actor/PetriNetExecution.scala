...
object PetriNetExecution {
  ...
  def eventSource[S]: ExecutionState[S] ⇒ Event ⇒ ExecutionState[S] = state ⇒ e ⇒ e match {
  ...
  case class ExecutionState[S](
     ...
     */
    def fireTransition[E](transition: Transition[Any, E, S], input: Any): (ExecutionState[S], Either[Job[S, E], String]) = {
