...
object PetriNetExecution {
   ...
   */
  def fireTransition[E, S](transition: Transition[Any, E, S], input: Any): InstanceState[S, Either[Job[S, E], String]] = s ⇒ {
   ...
   */
  def fireUnsafe[E, S](transition: Transition[Any, E, S], consume: Marking, input: Any): InstanceState[S, Job[S, E]] = s ⇒ {
  ...
  def fireAllEnabledTransitions[S]: InstanceState[S, Set[Job[S, _]]] = s ⇒ {
    val enabled = s.process.enabledParameters(s.availableMarking).find {
      case (t, markings) ⇒ t.isAutomated && !s.isBlockedReason(t.id).isDefined
    }
  ...
  case class ExceptionState(transitionId: Long, consecutiveFailureCount: Int, failureReason: String, failureStrategy: ExceptionStrategy)
