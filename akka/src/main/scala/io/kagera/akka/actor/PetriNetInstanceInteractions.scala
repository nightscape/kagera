...
object PetriNetInstanceInteractions {
  ...
  def hasAutomaticTransitions[S](topology: ExecutablePetriNet[S]): Marking ⇒ Boolean = marking ⇒ {
    marking.keySet.map(p ⇒ topology.outgoingTransitions(p)).foldLeft(Set.empty[Transition[_, _, _]]) {
  ...
  implicit class ActorRefAdditions(actor: ActorRef)(implicit actorSystem: ActorSystem) {
     ...
     */
    def fireAndConfirmFirst[S](topology: ExecutablePetriNet[S], msg: FireTransition)(implicit timeout: Timeout): Future[S] = {
      actor.ask(msg).map {
     ...
     */
    def fireAndConfirmAll[S](topology: ExecutablePetriNet[S], msg: FireTransition, waitForRetries: Boolean = false)(implicit timeout: Timeout): Future[S] = {
      Future {
     ...
     */
    def fireAndCollectResponses[S](topology: ExecutablePetriNet[S], msg: FireTransition, waitForRetries: Boolean = false)(implicit timeout: Timeout): Seq[Try[TransitionFired[S]]] = {
