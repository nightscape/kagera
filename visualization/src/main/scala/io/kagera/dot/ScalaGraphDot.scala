...
object ScalaGraphDot {
  ...
  def biPartiteNodeId[A, B]: LabelFn[Either[A, B]] = node ⇒ node match {
    case Left(a)  ⇒ a.toString
    case Right(b) ⇒ b.toString
  }

  def petriNetNodeShape[P, T]: DotAttrFn[Either[P, T]] = node ⇒ node match {
    case Left(nodeA)  ⇒ List(DotAttr("shape", "circle"))
    case Right(nodeB) ⇒ List(DotAttr("shape", "square"))
  }

  def markingShapeFn[P, T, M](marking: M)(implicit markingLike: MarkingLike[M, P]): DotAttrFn[Either[P, T]] = node ⇒ node match {
    case Left(nodeA) ⇒
  ...
  implicit class PetriNetVisualization[P, T](graph: Graph[Either[P, T], WLDiEdge]) {
    ...
    def toDot[M](marking: M)(implicit markingLike: MarkingLike[M, P]): String = toDot("Process", biPartiteNodeId, markingShapeFn(marking))
