...
object Test extends js.JSApp {
  ...
  def main(): Unit = {
    ...
    val bgLayer: Layer[FillStyle] = Layer(background) % (s ⇒ {
      case MouseEvent(MouseDown, button, _, _) ⇒ println(button); s
    })
    ...
    editor.addLayer(pointCloud, Layer[Set[Point]](d ⇒ pointCloudDrawable, MouseTools.insertTool[Point, Set[Point]](0)(p ⇒ p)))
