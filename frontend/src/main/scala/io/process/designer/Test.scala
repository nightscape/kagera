...
import io.process.designer.model.{ Foo, PetriNetModel, Node }
...
object Test extends js.JSApp {
  ...
  def main(): Unit = {
    ...
    val transitions = Node.set(PetriNetModel.transitions(10, bounds)).withTool(None, new SimpleMoveTool[VisualNode[Transition]](0))
     ...
     *    % { (p, s) => { case KeyEvent(Down, 'DELETE') => (p - s, Set.empty) } }
     *    *
     * }
     *
     */
