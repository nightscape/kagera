...
object MouseTools {
  ...
  def pick[T](implicit fn: Pickable[T]): Action[Set[T], Option[T]] = {
    case MouseEvent(MouseDown, button, p, _) ⇒ State { m ⇒ (m, m.find(e ⇒ fn.pick(e, p))) }
  }
  ...
  def drop[T](e: T): Action[Set[T], Option[T]] = {
    case MouseEvent(MouseUp, button, p, _) ⇒ State { m ⇒ (m, None) }
  }

  def insert[T](e: T): Action[Set[T], Option[T]] = {
    case MouseEvent(MouseUp, button, p, _) ⇒ State { m ⇒ (m + e, None) }
  }

  def showCoordinates[M](fillStyle: FillStyle): Action[M, Drawing] = {
    case MouseEvent(t, button, p, mods) ⇒
      val pr = p.round
