...
case class Layer[T: Drawable, S: Drawable](canvas: dom.html.Canvas, model: T, tool: MouseTool[T, S], updateFn: Option[T ⇒ Any])
...
class DomEditor(parent: dom.Element, val width: Int, val height: Int) {
  ...
  Seq("mousemove" -> Move, "mouseup" -> Up, "mousedown" -> Down).foreach {
  ...
  def addLayer[T: Drawable, S: Drawable](model: T, tool: Option[MouseTool[T, S]] = None, fn: Option[T ⇒ Any] = None) = {
