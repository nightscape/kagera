...
object PetriNetModel extends GraphVisuals {
  ...
  val transitionDrawable: Drawing =
    transitionShape.decorate(
      fill = Some("white"),
      stroke = Some("#7777ee")
    )

  def transitions(n: Int, bounds: Rectangle) = PointCloud.pointCloud(n, bounds).map(
    p ⇒ VisualNode[Transition](Transition(Random.nextLong()), transitionDrawable, p))

  def places(n: Int, bounds: Rectangle) = PointCloud.pointCloud(n, bounds).map(
    p ⇒ VisualNode[Place](Place(Random.nextLong()), placeDrawable, p))
