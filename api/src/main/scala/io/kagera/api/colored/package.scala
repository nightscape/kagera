...
package object colored {
  ...
  def coulouredMarkingLike[P]: MarkingLike[ColouredMarking[P], P] = new MarkingLike[ColouredMarking[P], P] {
    ...
    override def multiplicity(marking: ColouredMarking[P]): Marking[P] = marking.map { case (p, tokens) ⇒ (p, tokens.size.toLong) }.toMap
  ...
  def process(params: Seq[Arc]*): PTProcess[Place, Transition, Marking[Place]] =
    new ScalaGraphWrapper(Graph(params.reduce(_ ++ _): _*)) with SimpleExecutor[Place, Transition] with SimpleTokenGame[Place, Transition]
}
