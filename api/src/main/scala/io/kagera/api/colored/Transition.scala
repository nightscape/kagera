...
trait Transition {
  ...
  /**
   * The output type of this transition, that is the type of value this transition emits.
   * May be Unit if nothing is emitted.
   */
  type Output

  /**
   * The type of 'context' the transition requires. This is to allow transitions to access some
   * kind of global state to close over instead of just the in-adjacent marking.
   *
   ...
   * @return
   */
  def apply(inAdjacent: MultiSet[Place[_]], outAdjacent: MultiSet[Place[_]])(implicit executor: scala.concurrent.ExecutionContext): (ColoredMarking, Context, Input) ⇒ Future[(ColoredMarking, Output)]
