...
trait TransitionExecutor[S] {
  ...
  def fireTransition[Input, Output](t: Transition[Input, Output, S])(consume: Marking, state: S, input: Input): Task[(Marking, Output)] = {
    ...
    def handleFailure: PartialFunction[Throwable, Task[(Marking, Output)]] = {
      case e: Throwable ⇒ Task.fail(new TransitionFailedException(t, e))
    }
