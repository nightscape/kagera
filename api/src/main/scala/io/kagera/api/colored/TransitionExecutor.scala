...
trait TransitionExecutor[S] {
  ...
  def fireTransition[Input, Output](t: Transition[Input, Output, S])(consume: Marking, state: S, input: Input): Future[(Marking, Output)] = {
    ...
    def handleFailure: PartialFunction[Throwable, Future[(Marking, Output)]] = {
