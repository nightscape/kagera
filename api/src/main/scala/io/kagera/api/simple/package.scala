...
package object simple {
  ...
  def findEnabledTransitions[P, T](pn: PetriNet[P, T])(marking: Marking[P]): Set[T] = {
    ...
    /**
     * Slightly inefficient way of doing this, we don't need to check every out-adjacent transition.
     * Only those whose edge weight is smaller or equal to the token count
     */
