...
package object api {
  ...
  class ScalaGraphPetriNet[P, T](val innerGraph: BiPartiteGraph[P, T, WLDiEdge]) extends PetriNet[P, T] {
    ...
    override def inMarking(t: T): MultiSet[P] = innerGraph.get(t).incoming.map(e ⇒ e.source.asPlace -> e.weight.toInt).toMap
    override def outMarking(t: T): MultiSet[P] = innerGraph.get(t).outgoing.map(e ⇒ e.target.asPlace -> e.weight.toInt).toMap
