...
object ScalaGraph {
  ...
  implicit class DirectedBiPartiteGraphAdditions[A, B](val graph: BiPartiteGraph[A, B, WLDiEdge]) {
    ...
    def connectingEdgeAB(from: A, to: B): WLDiEdge[Either[A, B]] = graph.get(Left(from)).outgoing.find(_.target.value == Right(to)).get.toOuter

    def connectingEdgeBA(from: B, to: A): WLDiEdge[Either[A, B]] = graph.get(Right(from)).outgoing.find(_.target.value == Left(to)).get.toOuter
