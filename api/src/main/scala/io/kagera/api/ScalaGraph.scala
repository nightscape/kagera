...
object ScalaGraph {
  ...
  implicit class BiPartiteGraphAdditions[P, T](val process: BiPartiteGraph[P, T]) {
    ...
    val defaultShapFn: ShapeFn = node ⇒ node match {
    ...
    def toDot[M](marking: M)(implicit markingLike: MarkingLike[M, P]): String = toDotWithShapeFn(markingShapeFn(marking))
    ...
    private def toDotWithShapeFn(shapeFn: ShapeFn): String = {
      ...
      val root = DotRootGraph(
        ...
        attrList = List.empty) //List(DotAttr("attr_1", """"one""""), DotAttr("attr_2", "<two>")))
