...
package object ui {
  ...
  case class MouseEvent(eventType: MouseEventType, button: Int, location: Point, keyModifiers: Int) extends UIEvent with Transformable[MouseEvent] {
    override def transform(t: AffineTransform): MouseEvent = MouseEvent(eventType, button, t.apply(location), keyModifiers)
  }

  case class WheelEvent(deltaX: Double, deltaY: Double, deltaZ: Double, location: Point) extends UIEvent with CursorEvent
