...
object MouseTools {
  ...
  class SimpleMoveTool[T : Pickable : Movable](button: Int) extends DragTool[Set[T], T](button) {
    ...
    override def drag = (set, e) ⇒ (from, to) ⇒ {
  ...
  def showCoordinates[M](fillStyle: FillStyle): Action[M, Drawing] = {
    case MouseEvent(_, _, p, _) ⇒
      val pr = p.round
      State { m ⇒ (m, fill(fillStyle, Text(s"${pr.x},${pr.y}", p))) }
  }

  def insertTool[T, C <: Set[T]](button: Int)(fn: Point => T)(implicit builder: CanBuildFrom[Set[T], T, C]): UIHandler[C] = (elements: C) => {
    case MouseEvent(MouseDown, `button`, p, mods) => elements.++[T, C](Seq(fn(p)))
  }
}