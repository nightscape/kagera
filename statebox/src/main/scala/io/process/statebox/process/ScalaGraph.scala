...
object ScalaGraph {
  ...
  implicit class BiPartiteGraphAdditions[P, T](val process: BiPartiteGraph[P, T]) {
    ...
    def defaultShapFn: ShapeFn = node ⇒ node match {
      case Left(nodeA)  ⇒ List(DotAttr("shape", "circle")) //, DotAttr("label", " "), DotAttr("xlabel", nodeA.toString))
      case Right(nodeB) ⇒ List(DotAttr("shape", "square")) //, DotAttr("label", " "), DotAttr("xlabel", nodeB.toString))
    }
    ...
    def toDot() = {
      ...
      def myNodeTransformer(innerNode: BiPartiteGraph[P, T]#NodeT): Option[(DotGraph, DotNodeStmt)] = innerNode.value match {
      ...
      def myEdgeTransformer(innerEdge: BiPartiteGraph[P, T]#EdgeT): Option[(DotGraph, DotEdgeStmt)] = innerEdge.edge match {
        case WDiEdge(source, target, weight) ⇒
          Some((root,
            DotEdgeStmt(nodeId(source), nodeId(target),
