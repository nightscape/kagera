...
package object colored {
  ...
  implicit def %[A, B](places: (ColoredPlace { type Color = A }, ColoredPlace { type Color = B })): MarkingHolder[(A, B)] = new MarkingHolder[(A, B)] {
  ...
  implicit class PetriNetAdditions(val process: ColoredPetriNet) {
    ...
    def enabledTransitions(marking: Marking[ColoredPlace]): Set[Transition] = {
      ...
      marking.map {
        case (place, count) ⇒ process.get(place).outgoing.collect {
    ...
    def inMarking(t: Transition): Map[ColoredPlace, Int] = process.get(t).incoming.map(e ⇒ e.source.asPlace -> e.weight.toInt).toMap
    def outMarking(t: Transition): Map[ColoredPlace, Int] = process.get(t).outgoing.map(e ⇒ e.target.asPlace -> e.weight.toInt).toMap
