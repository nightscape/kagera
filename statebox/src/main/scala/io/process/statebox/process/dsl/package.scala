...
package object dsl {
  ...
  implicit def %[A, B](places: (Place { type Color = A }, Place { type Color = B })): MarkingHolder[(A, B)] = new MarkingHolder[(A, B)] {
  ...
  implicit class PetriNetAdditions(val process: ColoredPetriNet) {
    ...
    def outMarking(t: Transition): Map[Place, Long] = process.get(t).outgoing.map(e ⇒ e.target.asPlace -> e.weight).toMap
