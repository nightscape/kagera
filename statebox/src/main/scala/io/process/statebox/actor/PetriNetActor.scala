...
import akka.actor._
import io.process.statebox.actor.PetriNetActor.{ GetState, TransitionFired, NoFireableTransitions }
...
object PetriNetActor {
  ...
  def props[P, T, M](process: PTProcess[P, T, M], marking: M)(implicit markingLike: MarkingLike[M, P]) = Props(new PetriNetActor(process, marking))
...
class PetriNetActor[T, P, M](process: PTProcess[P, T, M], initialMarking: M)(implicit markingLike: MarkingLike[M, P]) extends Actor with ActorLogging {
